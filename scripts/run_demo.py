...
def run_role(role: str, steps: int = 10, policy: str = 'ucb1'):
    print(f"\n== {role.upper()} (policy={policy}) ==")
    runner = AgentRunner(role=role, policy=policy)
    for t in range(steps):
        out = runner.step(CTX)
        means = [round(m,3) for m in out['means']]
        print(f"[t{t}] strategy={out['strategy']:<22} reward={out['reward']:.3f} counts={out['counts']} means={means}")

if __name__ == '__main__':
    random.seed(42)
    for role in ['injector', 'exploiter', 'detector', 'patcher']:
        run_role(role, steps=10, policy='ucb1')
